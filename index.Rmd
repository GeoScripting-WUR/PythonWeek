...
After Miniconda is installed, you can install Spyder into the default Conda environment (by using the `anaconda` package channel):
...
This would install the `numpy` package. Note that this would install it into the default virtual environment. Conda is able to create any number of separate virtual environments, for example:

```{r, eval=FALSE, engine='bash'}
conda create --name astrolab python=2.7 numpy
```

This would create a new environment called `astrolab` with Python 2.7 and `numpy` installed into it. To list the available environments:
